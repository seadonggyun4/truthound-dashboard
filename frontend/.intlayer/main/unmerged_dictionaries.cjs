const _8SzASV83UGWAmS9N8VTm = require('../unmerged_dictionary/alerts.json');
const _DWiioKfqIdKP57y3ULts = require('../unmerged_dictionary/anomaly.json');
const _KRoWd9wBIIBMv7WbjZIE = require('../unmerged_dictionary/catalog.json');
const _Y7oZmA5LmDtL3JEgsUPM = require('../unmerged_dictionary/collaboration.json');
const _vCCVBp8kCGmTeHIzCwhA = require('../unmerged_dictionary/common.json');
const _iAiBly6ZxdWHhJsLlk7f = require('../unmerged_dictionary/crossAlerts.json');
const _qVbNqQq6pwYPWgya2g20 = require('../unmerged_dictionary/dashboard.json');
const _EqtELq9jioJ880u655Pn = require('../unmerged_dictionary/drift.json');
const _OG7aTbNq3ae16YBDiLtL = require('../unmerged_dictionary/driftMonitor.json');
const _Ua3ZVftySEXFAxqrkJjk = require('../unmerged_dictionary/errors.json');
const _rzrtvdsW6MA7HG6MZiqM = require('../unmerged_dictionary/glossary.json');
const _dmhiK1A81aPs1HigQOCq = require('../unmerged_dictionary/lineage.json');
const _3VLpCEMwdaORSSgN79Md = require('../unmerged_dictionary/maintenance.json');
const _ySyJyBKkKOgVeq0ogznB = require('../unmerged_dictionary/modelMonitoring.json');
const _CS4ZBheaDXved5UeTYiT = require('../unmerged_dictionary/nav.json');
const _sg3ZrUmJodT8xcUfcD8d = require('../unmerged_dictionary/notifications.json');
const _XWDjCahS3y3oXptSGSER = require('../unmerged_dictionary/notificationsAdvanced.json');
const _Igei5KSLe4KWTQUcFitu = require('../unmerged_dictionary/privacy.json');
const _4ns3sPukIi1W6H86cbxa = require('../unmerged_dictionary/profileComparison.json');
const _ixCqwaaO7BEY2QmXNuVQ = require('../unmerged_dictionary/reports.json');
const _CDyiw25c6kbrUOpz5byO = require('../unmerged_dictionary/ruleSuggestions.json');
const _xgn6YfRH8hT7OqP2mxRN = require('../unmerged_dictionary/schedules.json');
const _wQhNc7iOtkcmOJLyGpCv = require('../unmerged_dictionary/schemaEvolution.json');
const _ZOAdeBI7bXfBrnKogJeu = require('../unmerged_dictionary/settings.json');
const _mluQYzldy0M4oRk4zK6X = require('../unmerged_dictionary/sources.json');
const _Tk2hZa9MyMPkZgDZ7y4d = require('../unmerged_dictionary/validation.json');
const _yopRoCwvobwREyGLGqHI = require('../unmerged_dictionary/validators.json');
const _98KED6ES81RjX6bKkbvg = require('../unmerged_dictionary/versioning.json');

const dictionaries = {
  "alerts": _8SzASV83UGWAmS9N8VTm,
  "anomaly": _DWiioKfqIdKP57y3ULts,
  "catalog": _KRoWd9wBIIBMv7WbjZIE,
  "collaboration": _Y7oZmA5LmDtL3JEgsUPM,
  "common": _vCCVBp8kCGmTeHIzCwhA,
  "crossAlerts": _iAiBly6ZxdWHhJsLlk7f,
  "dashboard": _qVbNqQq6pwYPWgya2g20,
  "drift": _EqtELq9jioJ880u655Pn,
  "driftMonitor": _OG7aTbNq3ae16YBDiLtL,
  "errors": _Ua3ZVftySEXFAxqrkJjk,
  "glossary": _rzrtvdsW6MA7HG6MZiqM,
  "lineage": _dmhiK1A81aPs1HigQOCq,
  "maintenance": _3VLpCEMwdaORSSgN79Md,
  "modelMonitoring": _ySyJyBKkKOgVeq0ogznB,
  "nav": _CS4ZBheaDXved5UeTYiT,
  "notifications": _sg3ZrUmJodT8xcUfcD8d,
  "notificationsAdvanced": _XWDjCahS3y3oXptSGSER,
  "privacy": _Igei5KSLe4KWTQUcFitu,
  "profileComparison": _4ns3sPukIi1W6H86cbxa,
  "reports": _ixCqwaaO7BEY2QmXNuVQ,
  "ruleSuggestions": _CDyiw25c6kbrUOpz5byO,
  "schedules": _xgn6YfRH8hT7OqP2mxRN,
  "schemaEvolution": _wQhNc7iOtkcmOJLyGpCv,
  "settings": _ZOAdeBI7bXfBrnKogJeu,
  "sources": _mluQYzldy0M4oRk4zK6X,
  "validation": _Tk2hZa9MyMPkZgDZ7y4d,
  "validators": _yopRoCwvobwREyGLGqHI,
  "versioning": _98KED6ES81RjX6bKkbvg
};
const getUnmergedDictionaries = () => dictionaries;

module.exports.getUnmergedDictionaries = getUnmergedDictionaries;
module.exports = dictionaries;
